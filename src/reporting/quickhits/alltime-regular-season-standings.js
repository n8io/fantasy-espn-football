...
const getSeeds = ({ homeTeam, homeTeam: { tags: hTags }, awayTeam, awayTeam: { tags: aTags } }) => {
  const hSeed = hTags.find(t => t.startsWith('SEED_'));
  const aSeed = aTags.find(t => t.startsWith('SEED_'));
  ...
  return [
    {
      season,
      team: home,
      place: ~~hSeed.replace(/SEED[_]/g, ''),
    },
    {
      season,
      team: away,
      place: ~~aSeed.replace(/SEED[_]/g, ''),
    },
  ];
};

const isSeedMatchup = ({ homeTeam: { tags } }) => tags.some(t => t.startsWith('SEED_'));
...
const fileName = `regular-season-standings.${YEARS_BACK ? `last-${`00${YEARS_BACK}`.slice(-2)}-years` : 'alltime'}`;
