...
const getLosingestTeams = () => {
  const winningestTeams = Object.keys(teamRecords)
    .map(key => ({
      ...
      lossPct: math.round(teamRecords[key].losses / (teamRecords[key].wins + teamRecords[key].losses) * 100, 0),
    ...
    .map((team, index) => {
      ...
      return {
        [index + 1]: `${firstName} ${lastName} (${name}) with a ${team.lossPct}% loss percentage`,
      };
    })
