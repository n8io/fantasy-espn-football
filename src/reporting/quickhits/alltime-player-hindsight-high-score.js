...
export default () => {
  const scores = ACTIVITIES.filter(
    ({ trophy, details: { points } }) => trophy && trophy.type === TT.WEEKLY_HINDSIGHT && points > 0
  )
  ...
  return activities.map(({ season, trophy: { team: { id } }, details: { week, points, player } }, index) => {
    ...
    const suffix = `${realName} (${member.name}) left ${playerName} on the bench when he scored ${math.round(
      scores[index],
      2
    )} points`;
