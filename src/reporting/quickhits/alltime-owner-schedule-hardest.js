...
const getLosingestTeams = () => {
  const losingestTeams = Object.keys(teamRecords)
    ...
    .map(tr => ({
      ...tr,
      avgPointsAgainst: math.round(tr.pointsAgainst / (tr.wins + tr.losses), 2),
    }))
    .sort((a, b) => {
      if (a.avgPointsAgainst > b.avgPointsAgainst) {
        return -1;
      } else if (a.avgPointsAgainst < b.avgPointsAgainst) {
        return 1;
      }
    ...
    .map((team, index) => {
      ...
      return {
        [index + 1]: `${firstName} ${lastName} (${name}) with an avg of ${team.avgPointsAgainst} points against`,
      };
    });
